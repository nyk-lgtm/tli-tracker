 ...
 * Renders Pulse, Efficiency Trend, and Loot Distribution charts
 */
 ...
 */
export function renderPulseChart(container, maps, currentMap = null) {
    if (!container) return;
    ...
    const barsHTML = recentMaps.map((map) => {
        ...
        return `<div class="pulse-bar${liveClass}" style="height: ${minHeight}%" data-value="${formatCompact(value)}"></div>`;
    }).join('');
 ...
 */
export function renderEfficiencyChart(container, maps, sessionDuration, currentValue) {
    if (!container) return;
 ...
 */
export function renderDonutChart(container, drops) {
    if (!container) return;
    ...
    container.innerHTML = `
        <div class="donut-chart">
            <div class="donut-ring" style="background: conic-gradient(${gradientStops})">
                <div class="donut-center">${formatCompact(total)}</div>
            </div>
